...
def book_page():
    ...
    if not slots:
        form_html='<p style="color:var(--muted);font-size:13px">No available slots right now. Email us to schedule.</p>'
    else:
        slots_html="".join(f'<div class="slot" data-start="{s["start"]}" onclick="pick(this)">{s["display"]}</div>' for s in slots)
        form_html=f"""
        <div id="bform">
          <div class="bf"><label>Your Name</label><input type="text" id="b-name" placeholder="Jane Smith"/></div>
          <div class="bf"><label>Business Name</label><input type="text" id="b-biz" placeholder="Your Company"/></div>
          <div class="bf"><label>Email</label><input type="email" id="b-email" placeholder="you@example.com"/></div>
          <div class="bf"><label>Choose a Time (CT)</label><div class="slots">{slots_html}</div></div>
          <input type="hidden" id="b-slot"/>
          <div id="b-err" style="font-size:12px;color:#f43f5e;margin-bottom:10px"></div>
          <button class="bbtn" id="b-btn" onclick="submitBook()">Book My Free Call &rarr;</button>
        </div>
        <div id="b-success" style="display:none;text-align:center;padding:30px 0">
          <div style="font-size:40px;margin-bottom:16px">&#127881;</div>
          <h2 style="font-size:20px;font-weight:800;color:#22c55e;margin-bottom:8px">You're booked!</h2>
          <p style="color:rgba(255,255,255,.5);font-size:13px;line-height:1.7">Check your email for your confirmation<br>and Google Meet link. Talk soon!</p>
        </div>
        <script>
        function pick(el){{document.querySelectorAll('.slot').forEach(s=>s.classList.remove('sel'));el.classList.add('sel');document.getElementById('b-slot').value=el.dataset.start;}}
        async function submitBook(){{
          ...
          if(!name||!email||!slot){{err.textContent='Please fill in all fields and select a time.';return;}}
          const btn=document.getElementById('b-btn');btn.disabled=true;btn.textContent='Booking...';
          try{{
            const res=await fetch('/book',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{name,business:biz,email,start:slot}})}});
            const d=await res.json();
          ...
          }}catch(e){{err.textContent=e.message;btn.disabled=false;btn.textContent='Book My Free Call \u2192';}}
        }}
        </script>"""

    return HTMLResponse(f"""<!DOCTYPE html><html><head><meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Book a Call · Lumera</title>
<link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@600;700;800&display=swap" rel="stylesheet"/>
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
:root{{--black:#080808;--surface:#111;--border:rgba(255,255,255,0.07);--border2:rgba(255,255,255,0.14);
  --text:#fff;--muted:rgba(255,255,255,0.45);--indigo:#6366f1;--blue:#3b82f6;--red:#f43f5e;
  --grad:linear-gradient(135deg,#3b82f6,#6366f1);--font:'Montserrat',sans-serif}}
body{{font-family:var(--font);background:var(--black);color:var(--text);min-height:100vh;
  background-image:url("{NOISE_SVG}"),radial-gradient(ellipse,rgba(99,102,241,0.08) 0%,transparent 70%);
...
.card{{background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:36px;width:100%;max-width:560px;}}
.logo-wrap{{display:flex;align-items:center;gap:10px;margin-bottom:24px;}}
.logo-icon{{width:36px;height:36px;border-radius:10px;background:var(--grad);display:flex;align-items:center;justify-content:center;font-size:18px;box-shadow:0 4px 14px rgba(99,102,241,.4);}}
.logo-text{{font-size:15px;font-weight:800;background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}}
h2{{font-size:20px;font-weight:800;margin-bottom:4px;}}
.sub{{font-size:12px;color:var(--muted);margin-bottom:28px;}}
label{{display:block;font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.1em;margin-bottom:6px;}}
input{{width:100%;background:var(--black);border:1px solid var(--border2);border-radius:10px;padding:10px 13px;color:var(--text);font-family:var(--font);font-size:13px;outline:none;}}
input:focus{{border-color:var(--indigo);box-shadow:0 0 0 3px rgba(99,102,241,.12);}}
.bf{{margin-bottom:14px;}}
.slots{{max-height:260px;overflow-y:auto;margin-bottom:14px;}}
.slot{{padding:10px 14px;border:1px solid var(--border);border-radius:10px;margin-bottom:7px;cursor:pointer;font-size:12px;font-weight:600;color:var(--muted);transition:all .15s;}}
.slot:hover,.slot.sel{{border-color:var(--indigo);color:var(--text);background:rgba(99,102,241,.1);}}
.bbtn{{width:100%;padding:13px;background:var(--grad);color:white;border:none;border-radius:10px;font-family:var(--font);font-weight:700;font-size:13px;cursor:pointer;box-shadow:0 4px 14px rgba(99,102,241,.35);}}
.bbtn:hover{{opacity:.9;}} .bbtn:disabled{{opacity:.5;cursor:wait;}}
</style></head><body>
<div class="card">
  <div class="logo-wrap">
  ...
  <div class="sub">30 min &nbsp;·&nbsp; no pitch &nbsp;·&nbsp; Mon&ndash;Fri 9am&ndash;5pm CT</div>
  {form_html}
</div></body></html>""")
